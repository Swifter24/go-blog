...
func (j *JWT) CreateRefreshClaims(baseClaims request.BaseClaims) request.JwtCustomRefreshClaim {
	ep, _ := ParseDuration(global.Config.Jwt.RefreshTokenExpiryTime) // 获取过期时间
	claims := request.JwtCustomRefreshClaim{
...
func (j *JWT) CreateRefreshToken(claims request.JwtCustomRefreshClaim) (string, error) {
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, claims) // 创建新的 JWT Token
...
func (j *JWT) ParseRefreshToken(tokenString string) (*request.JwtCustomRefreshClaim, error) {
	claims, err := j.parseToken(tokenString, &request.JwtCustomRefreshClaim{}, j.RefreshTokenSecret) // 解析 Token
	if err != nil {
		return nil, err
	}
	if refreshClaims, ok := claims.(*request.JwtCustomRefreshClaim); ok { // 确保解析出的 Claims 类型正确
